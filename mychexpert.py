...
class CheXpert(Dataset):
    ...
    def __init__(
        ...
        assert (
            image_root_path != ""
        ), "You need to pass the correct location for the dataset!"
        ...
        self._images_list = [
            image_root_path + path for path in self.df["Path"].tolist()
        ]
...
if __name__ == "__main__":
    ...
    trainloader = torch.utils.data.DataLoader(
        traindSet, batch_size=32, num_workers=2, drop_last=True, shuffle=True
    )
    testloader = torch.utils.data.DataLoader(
        testSet, batch_size=32, num_workers=2, drop_last=False, shuffle=False
    )
